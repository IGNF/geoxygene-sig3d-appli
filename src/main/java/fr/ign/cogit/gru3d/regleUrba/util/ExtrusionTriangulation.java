...
public class ExtrusionTriangulation {
	 ...
	 * @return
	 */
	public static ISolid process(IGeometry geom, IGeometry geomDist, double zmin, DistanceFHauteur dfh) {
		...
		for (IOrientableSurface os : lOS) {
			...
			for (int i = 0; i < nbFace; i++) {
				...
				double xC = (pointTri.get(0).getX() + pointTri.get(1).getX() + pointTri.get(2).getX()) / 3;
				double yC = (pointTri.get(0).getY() + pointTri.get(1).getY() + pointTri.get(2).getY()) / 3;
				double zC = (pointTri.get(0).getZ() + pointTri.get(1).getZ() + pointTri.get(2).getZ()) / 3;
			...
			IMultiSurface<IOrientableSurface> iMS1 = new GM_MultiSurface<IOrientableSurface>(lOSTemp);
			IMultiSurface<IOrientableSurface> iMS2 = (IMultiSurface<IOrientableSurface>) iMS1.clone();
	...
	private static IMultiSurface<IOrientableSurface> generateWall(IPolygon surf, double zmin) {
