...
public class ParisProposition {
	...
	private static Batiment generateRandomBuilding(Parcelle p) {
		...
		while (iteration++ < 200) {
			...
			// Quel toit ?
			IMultiSurface<IPolygon> bGeom = generateRoof(poly, p.getGeom().coord().get(0).getZ(), fB);
	 ...
	 * @return
	 */
	private static IPolygon propositionPolygon(FormeBatiment fB, IDirectPosition iDirectPosition) {
		...
		if (fB.equals(FormeBatiment.Rectangle)) {
			...
			return (IPolygon) ShapeFactory.createRectangle(iDirectPosition, largeur, hauteur);
			...
			return (IPolygon) ShapeFactory.createL(iDirectPosition, largeur, hauteur,
					largeur / ((int) (2 + Math.random() * 3)), hauteur / ((int) (2 + Math.random() * 3)));
			...
			return (IPolygon) ShapeFactory.createEscalier2(iDirectPosition, largeur, hauteur, largeur1, hauteur1,
					largeur1, hauteur1);
	...
	private static IPolygon propositionRandomRotation(IPolygon poly) {
		...
		try {
			poly = (IPolygon) JtsGeOxygene
					.makeGeOxygeneGeom(JtsUtil.rotation((Polygon) JtsGeOxygene.makeJtsGeom(poly), angle));
		} catch (Exception e) {
	...
	private static IPolygon propositionOrientedRotation(IPolygon poly, Parcelle p) {
		...
		try {
			...
			double orientationTroncon = JtsUtil.projectionPointOrientationTroncon(ls.coord().get(0), ls);
	...
	private static IMultiSurface<IPolygon> generateRoof(IPolygon poly, double zMin, FormeBatiment fB) {
	...
	public static List<ILineString> determinePignon(FormeBatiment fB, int nbPans, IPolygon geom) {
